...
export class SqliteDeductionGoalsRepository {
  ...
  public async insertGoal(goal: DeductionGoal): Promise<void> {
    await this.database.insertInto("deduction_goals").values({
      ...
      narrative_kind: goal.narrativeKind,
      scale: goal.scale,
      plant_chapter_sequence: goal.plantChapterSequence ?? null,
      payoff_chapter_sequence: goal.payoffChapterSequence ?? null,
...
function mapGoal(row: ProjectDatabase["deduction_goals"]): DeductionGoal {
  return {
    ...
    narrativeKind: row.narrative_kind,
    scale: row.scale,
    createdAtMs: row.created_at_ms,
    updatedAtMs: row.updated_at_ms,
    ...(row.plant_chapter_sequence === null ? {} : { plantChapterSequence: row.plant_chapter_sequence }),
    ...(row.payoff_chapter_sequence === null ? {} : { payoffChapterSequence: row.payoff_chapter_sequence }),
