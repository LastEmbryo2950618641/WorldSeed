...
6. 当前有效内容必须沿前置修订、生效时间和当前入口连续，不能因上下文压缩、旧资料相似或长期未出现而回退；
...
18. 基础规则、用户规则、设定集和参考资料分层管理，用户规则在明确适用范围内优先于平台默认创作建议，范围外或 AI无法判断时回退默认规则；四层内容都必须选择性读取并记录来源；
...
- 用户可以挂载第三方文献、同人作品和其他参考资料；有索引时按索引选择性检索，无索引时 AI使用可用查询能力自行查找并记录实际来源。

#### 时间与空间连续性

- 持久化图与模型上下文分离，上下文压缩不能修改世界；
- 已确认内容持续有效，直到 AI建立明确的后续变化或主动修订；
- 任何改变当前有效内容的修改都必须由 AI连接前置修订、生效时间和新的当前入口；
...
- 只有候选原文、图修订、检索投影、正文结算和前沿结算全部通过 AI提交复核后，正文和图作用域才共同进入权威可见范围；
...
- 世界时间推进后，自治世界推进器在可调预算内选择少量局部进行后台推演，不遍历和更新整张图；
- 后台结果直接进入持久化世界图，但只有沿合理影响路径到达当前场景后才允许进入正文或人物认知；
- 长期未推进的局部在重新进入或查询前执行惰性追赶，从最后有效状态恢复到当前世界时间；
...
每轮正式正文都必须由 AI生成一个连续章节序号和章节名，并以 `章节正文/<章节名>.md` 保存。正文第一行必须是与文件名一致的 `第一章 xxxx` 形式标题；该标题是正文的一部分，必须进入第一个 `NarrativeSourceUnit` 和图结算映射。章节序号和章节名由 AI根据上一章、当前规则快照、世界时间和本轮内容决定，代码只保存文件、检查路径冲突并维护索引。
...
`visibility` 是基础设施可见性，不表示故事真假。候选正文第一次保存时为 `pending`，只进入应用内部存储及恢复与结算专用索引，不在用户文件树中伪装成 Markdown 文件。普通世界召回、后续推演依赖和正式小说阅读默认只能读取 `committed`。完成依赖及时空审计、待结算图治理、不可变修订、正文单元结算、自审复核和演化前沿结算后，AI才指示代码把候选正文及同一 `scopeId` 的待结算图修订转为 `committed`，并将该正文版本发布到用户工作目录。被 AI放弃或替代的候选及图修订转为 `retired`，继续不可变保留，但不能成为世界事实依据。

代码只按可见性和 `scopeId` 隔离检索范围，不判断正文或图修改是否合理；转为 `committed` 的语义决定仍由 AI在完整闭环后作出。预算耗尽或结算失败时记录保持 `pending`，下一次只能在显式恢复该任务时读取，不能污染普通世界查询。该设计规定语义可见范围，但不承诺正文、索引和图存储的原子耐久提交；原子性仍属于后续实现范围。
...
`chapterId` 表示稳定章节身份，`sourceId` 表示某次不可变正文版本。用户可以编辑已经提交的章节 Markdown，但这不是普通文本覆盖：系统为同一 `chapterId` 创建新的 `sourceId`，通过 `predecessorSourceId` 连接前一版本，并为新版本创建正文修订任务，重新切分原文单元、更新图结算映射、审计世界事实变化并经过提交复核。用户可以修改表达，但不能通过编辑器直接让已提交世界状态回退。
...
type AIDecisionRecord = {
  ...
  continuity?: {
    ...
    effectiveWorldTimeAnchorIds: string[]
    currentEntryAnchorIds: string[]
    explanation: string
  }
  followUp?: string
  visibility: "pending" | "committed" | "retired"
...
任何修改只要改变了后续推演应采用的当前有效内容，AI就必须填写 `continuity`。它说明本次变化继承或替代哪些既有修订、在什么世界时间范围生效、未来从哪些当前入口重新发现结果，以及为什么变化前后能够连续。字段本身不规定状态类型；婚姻、位置、统治、比赛结果、规则变化或任何其他内容都使用同一证明方式。
...
代码只更新文件任务的数量、状态、错误和进度，不决定归档是否符合世界语义。章节提交仍由 AI阶段和 `commit_review` 决定；`write_chapter` 只能把已经批准的 `pending` Markdown 写入或转换为对应可见性。
...
这些问题不要求固定状态字段或领域边类型。AI可以继续在原节点上更新当前位置、关系或其他资料，也可以建立新的局部结构；无论选择哪种表达，代码都机械保留 `GraphRevision`，AI都必须在 `AIDecisionRecord.continuity` 中给出连续性。
...
flowchart TD
    ...
    FN --> G{"旧依赖是否闭合且场景时空连续？"}
    G -->|否| H["补充召回；计划外新内容返回出现规划；否则降为不确定或世界修改"]
    H --> C
    G -->|是| I["AI修订并确认候选内容"]
    ...
    L --> M["AI依据自治原则自审"]
    M -->|否决或需修改| L
    M -->|批准| N["AI指示代码在本轮 pending 图作用域执行修改"]
    N --> O["AI重新读取默认图与 pending 叠加后的局部图"]
    O --> P["AI用实际查询、依赖与时空连续性复核"]
    P -->|不满意| L
    P -->|满意| Q["为全部原文单元形成图结算映射并补齐检索投影"]
    Q --> QA{"结算覆盖与语义复核是否完整？"}
    QA -->|否| L
...
闭环不是“生成后自动写入固定字段”，而是 AI 在每轮开始前从持久化图恢复本轮依赖和场景时空锚点，完成当前场景自治扫描后，对出现压力执行独立规划与审查，再形成不对用户发布的草稿。草稿如果引入了计划外新内容，必须返回出现规划与身份召回；如果依赖审计不通过，则继续召回和修订，不能让未解析的既有指代、无依据新身份或无锚点场景直接进入正式正文。

审计通过后，完整候选正文首先以 `pending` 状态单独保存，再由 AI重新理解全部实际出现内容并在同一待结算作用域治理图。图治理复核通过后，AI必须为每个不可变原文单元形成 `NarrativeSettlementRecord` 映射并执行语义覆盖自审，然后对所有受影响局部执行演化前沿结算；全部通过后候选正文和本轮图修订才共同转为 `committed`、进入权威检索并发布正式输出。正文内容不会因为没有预定义字段、当前看似不重要或未来上下文压缩而丢失；AI 可以在既有图基础上为它们建立适合当前世界的表达。
...
图自身通过节点、连接、高层抽象和多入口结构承担主要的语义索引。代码必须对所有节点、连接、它们可选携带的信息、不可变修订、正文结算记录以及持久化原始资料提供无语义的全局机械检索，至少能够支持精确内容、相似内容和邻接入口的候选召回。精确召回使用规范载荷、`canonicalDigest` 和 `exactKeys`，相似召回使用 AI生成的 `semanticTexts`；代码不从这些文本推断世界类型。任何内容写入或重构后，都必须在提交前处于可检索状态，但普通语义事务只召回 `committed` 投影；显式恢复任务才读取同一作用域的 `pending` 投影。代码只返回候选，AI决定候选是否相关、如何解释和沿哪个方向继续读取。
...
相似度、写入顺序和文本出现顺序都不能单独决定当前有效内容。AI必须优先恢复相关当前入口及其 `AIDecisionRecord.continuity`，沿 `predecessorRevisionIds` 理解变化链，并用 `effectiveWorldTimeAnchorIds` 判断该结果是否适用于当前问题。若当前入口、修订链和世界时间不能闭合，AI只能继续召回或保留不确定性，不能从相似候选中任选一个状态。
...
AI批准后，指示代码在本轮 `pending` 作用域应用它决定的新增、编辑、迁移和归档重构。代码忠实执行，不作语义裁决。
...
2. `rule_assembly` 读取不可修改的基础规则摘要、适用的用户规则版本和 Skill manifest，形成不可变 `RuleSnapshot`；
3. `source_retrieval` 按任务选择性读取设定集片段和参考资料；没有索引时执行可用的文件或外部查询，并把实际来源加入读取集合；
...
13. `frontier_settlement` 结算受影响局部的前沿身份、最后推演时间和推迟记录；
14. `commit_review` 汇总前述阶段证据，决定提交、继续修订或转为 `retired`。
...
`chapter_naming` 的提示词必须要求 AI：读取最后一个 `committed` 章节的标题和文本顺序；结合当前规则快照、当前世界时间、地点和草稿内容；生成唯一且适合文件名的 `第一章 xxxx` 形式标题；把完全相同的标题写入章节文件名和正文第一行；说明标题与上一章的延续依据；不能使用 `pending` 或已废弃章节作为正式历史依据。若标题包含文件系统保留字符，AI必须自行重新命名，代码只负责拒绝非法路径、保存合法 Markdown 路径、维护文件版本和索引，不替 AI决定标题含义。
...
`maxTurnInputTokens` 和 `maxTurnOutputTokens` 统计本轮所有主推演模型调用的累计输入与输出，不是单次上下文窗口。后台推进继续使用独立的 `WorldEvolutionProfile` 总预算，防止后台工作挤占用户当前正文的主预算。
...
底层动态图能够保存和恢复世界，但不会自行决定哪些局部在用户视野之外继续变化。自治世界推进器运行在持久化图之上，在世界时间前进时选择有限数量的局部图执行独立语义推演，并把结果写回同一张图。
...
5. 为每个仍活跃的局部建立可返回的最后推演时间锚点，并确保能够和当前世界时间锚点共同读取。

最后推演时间锚点是前沿局部必须满足的结构条件，不再是可选提示。时间本身仍由 AI使用当前世界适合的节点和连接表达，代码不规定格式或边类型。

结算时即使 AI判断某个受影响局部不需要进入前沿，也必须留下判断原因。这样演化前沿不会依赖模型偶然记起，而是每轮图治理的固定语义步骤。

### 14.4 触发与候选选择

正式场景完成后，AI根据场景时空锚点更新配置指向的当前世界时间锚点，并判断世界时间是否实际前进。只有发生了有效时间推进，或某次图变化明确要求其他局部重新评估时，才触发后台推进。

推进器从演化前沿和全局索引召回候选，由 AI在预算内选择最值得继续推演的局部。选择目标是尽量减少世界冻结、因果中断和长期未更新造成的不合理，而不是优先选择离主角最近的内容。

每个未入选候选都必须在原有前沿身份上记录本次考虑位置、连续推迟次数和 AI给出的推迟原因。候选选择必须同时考虑局部因果压力、距最后推演时间、连续推迟次数和本轮影响价值；这些含义由 AI解释，但不能只反复选择最熟悉或最靠近主角的局部。
...
后台预算不可能持续更新整个世界。用户准备进入、使用或查询一个长期未加载的局部时，AI必须先读取该局部最后有效状态、其最后时空锚点、当前世界时间以及期间可能影响它的其他变化，再在预算内批量推演缺失时期。
...
type WorldEvolutionProfile = {
  ...
  currentWorldTimeAnchorId: string
...
13. 所有节点、连接、携带信息、不可变修订、正文结算记录和独立小说原文都通过 `RetrievalProjection` 进入对应可见性范围的机械索引，并在提交后可被普通语义事务检索。
...
35. 世界时间前进后，AI在可调预算内选择少量用户视野外的局部执行后台推演，而不是遍历整张图。
...
39. 长期未加载的局部在重新进入、使用或查询前执行惰性追赶，并在预算内恢复到当前世界时间。
...
48. 每个活跃前沿局部都能返回其最后推演时间锚点，并能与配置指向的当前世界时间锚点共同读取。
...
56. 任何改变当前有效内容的修改都连接前置修订、生效世界时间和新的当前入口；历史相似度或写入顺序不能替代该连续性证明。
...
74. 参考资料有索引时优先使用索引，无索引时 AI可以自主查询并保存实际来源；外部内容不会因查询而自动成为作品世界事实。
...
78. 正式章节文件名和正文首行使用 AI生成的同一 `第一章 xxxx` 标题，标题属于第一个原文单元并参与结算映射。
79. `chapter_naming` 读取最后已提交章节和当前规则快照生成标题；只读查询、后台演化和失败任务不生成正式章节。
80. 用户编辑已提交章节时必须生成正文修订、保留旧版本并重新执行原文结算和图治理，不能直接覆盖已提交世界事实。
...
22. 建立多个彼此分离但仍可能发展的局部，只让用户关注其中一个，并检查世界时间前进后其他局部是否按预算自主变化；
...
26. 长时间不推进某个局部，再重新进入它，检查惰性追赶是否从最后有效状态恢复到当前世界时间；
...
35. 人为移除一个活跃前沿的最后推演时间锚点，检查 AI是否拒绝无时间基准推进，并在结算中修复可返回的时间锚点；
...
45. 在当前场景放置一个仍可能自主发展的局部，同时让用户输入与它无关，检查正文草稿前是否仍执行自治扫描并采用候选或留下有依据的不采用原因；
46. 分别耗尽主推演模型调用、输入 token、输出 token、耗时和各阶段循环次数，检查系统是否先削减可选扩展，并在必要闭环未完成时停止正式发布；
47. 连续推迟同一批前沿超过 `maxConsecutiveFrontierDeferrals`，检查后续轮次是否保留逾期尝试份额，同时不强迫资料不足的局部编造变化；
48. 在固定长篇测试集上统计正文结算覆盖、修订保留、历史召回、当前状态回退、重复身份、预算超限、延迟和自主信号多样性，并与量化门槛比较。
49. 在结算前尝试从普通世界查询读取 `pending` 正文、图修订和决定记录，检查它们不会进入权威候选；显式恢复同一任务时检查它们能够继续使用。
50. 为节点、连接、修订、原文和结算记录提供不同结构的任意载荷，检查规范摘要、精确键、语义文本和可见性都能建立并完成召回。
51. 删除或伪造一个 Prompt Contract 阶段结果，检查后续阶段不能直接提交；让依赖审计发现未读取资料，检查它只能请求实际召回或保留不确定性。
52. 使用固定机位、景别、视角和笔风约束生成正文，并由独立评估器逐项评分；再输入与世界矛盾的用户说法，检查 AI保留用户意图但不无依据提交为真相。
53. 对有充分自治候选和无自治候选的章节分别进行盲评，检查前者的世界运行感、因果依据、人物认知边界和主线占比达到量化门槛，后者不为凑指标强行制造变化。
54. 连续生成多轮正式正文，检查每轮都在 `章节正文` 生成唯一 Markdown 文件，并且文件名和正文首行使用同一 AI章节标题；
55. 清空上下文后继续生成下一章，检查 `chapter_naming` 能从最后已提交章节和规则快照恢复章节顺序与命名连续性；
56. 在章节生成的不同阶段暂停、失败和恢复，检查 `pending` 章节文件不会进入普通正文阅读或世界事实召回；
57. 编辑已经提交的章节 Markdown，检查旧版本、原文单元、图结算映射和世界状态都保留，并且新版本必须重新审计后才能提交；
58. 尝试在项目根目录创建第六个顶级目录或写入非 Markdown 文件，检查工作区协议和存储层都拒绝该操作。
59. 删除、归档、恢复和章节写入各产生 `WorkspaceOperation`，检查进度字段与真实完成单元一致，失败后能够记录原因并恢复；
60. 删除或损坏 `ProjectManifest` 中的固定目录，检查打开项目时进入明确诊断/修复流程，不静默生成替代目录或读取不明文件；
61. 编辑已提交章节后只执行普通保存，检查正式章节和世界图不变；点击“提交章节修订”后才进入完整结算流程；
