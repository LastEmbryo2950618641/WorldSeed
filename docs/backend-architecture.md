...
第一阶段默认提供一个通用 `DeepSeek` 配置，初始模型为 `deepseek-chat`，具体可用模型由服务端模型列表选择。模型、base URL、超时、最大调用次数和 token 上限都进入项目运行配置与任务预算快照。应用不设置整轮累计输出截止线；已知模型的正文阶段按用户字数范围计算单次输出预算，非正文阶段使用阶段级结构化护栏，未知模型不硬编码供应商能力。
