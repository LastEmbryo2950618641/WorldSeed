...
平台提供的默认 Markdown 文件可以被用户查看，但属于基础规则时只读。用户规则、设定集、参考文件、章节正文、描写规则和笔风规则中的 Markdown 可编辑；章节正文的编辑会触发正文修订和重新结算，不能静默覆盖旧版本。用户可以选择直接提交，或先让 AI 审核后再提交；审核只提供建议，用户可以忽略建议并强制提交。提交后的正文立即成为新的正式版本，图同步状态单独展示并可恢复。
