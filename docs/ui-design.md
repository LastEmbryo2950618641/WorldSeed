...
本文只定义客户端工作台的布局、可见信息、交互入口和 UI 验收标准。节点、连接、修订、检索、章节结算、规则快照、项目清单和任务进度的数据契约由 [底层动态图设计](system-design.md) 定义；规则和资料的注入方式由 [规则与资料分层](rule-and-source-layers.md) 定义，世界内容的出现时机由 [世界内容出现与生成规则](world-emergence-rules.md) 定义。底层文档不反向规定菜单、面板或视觉布局。
...
“推演执行”配置单轮最大模型调用、模型上下文窗口、主动压缩阈值、最长墙钟时间与最大检索轮次；输出 Token 不设置应用级累计上限，由模型接口自身能力决定。“资料检索”配置每轮读取请求、单请求候选、最大检索深度与证据 Token 上限。“世界图”配置直接入度/出度上限、合并预警阈值、推荐和最大展开深度、局部访问节点和连接上限；布局固定显示为“分层避碰”，该字段只影响显示，不改变世界事实。

项目参数写入应用内部的项目 SQLite，不在用户工作目录创建 JSON、数据库或任务文件。默认单轮预算为 `63` 次模型调用、`780000` 毫秒和 `4` 次检索轮次；这些值按基础执行路径保留约 30% 的失败重试冗余。模型上下文窗口默认为 `1000000` Token，系统在 `95%` 即 `950000` Token 处停止继续扩张并进入自有压缩流程，不能把超限后的截断交给模型供应商。应用不设置累计输入、输出 Token 截止线，只持续计量实际消耗；对已知模型传递供应商的单次输出上限，未知模型不硬编码 `max_tokens`。保存后的设置从下一轮推演开始生效；已经运行的任务继续使用启动时冻结的配置快照。模型服务入口在同一设置导航中可达，但模型 Profile 属于应用级配置，API Key 继续使用系统安全存储，不写入项目数据库。
...
用户可以暂停或停止任务。停止只保留 `pending` 和阶段记录，不将其伪装为世界事实；继续任务时使用原 `RuleSnapshot`，不会在半轮中途换规则。
...
sequenceDiagram
    ...
    U->>C: 输入本轮故事发展与表现要求
    C->>A: 开始推演
    A->>R: 显示 interpret 和 rule_assembly
    A->>L: 选择性读取适用规则与资料片段
    A->>G: 召回当前场景和必要局部
...
- 蓝色：当前读取、进行中或可继续；
- 绿色：已通过审查并 `committed`；
- 黄色：`pending`、延后、等待读取或需要修订；
- 红色：预算失败、规则冲突未解决、索引损坏或任务失败。
