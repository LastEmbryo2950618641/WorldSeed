...
- 文件名格式为 `第一章 xxxx.md`，其中章节序号和 `xxxx` 都由 AI根据上一章、当前大纲、世界时间和本轮推演内容生成；
...
用户可以写普通 Markdown，不强制使用结构化字段。系统只解析必要的适用范围、优先级语义、生效时间和引用信息，世界内容仍由 AI在通用图中解释。
...
8. 设定集、参考文件和表现规则以 Skill manifest 加选择性片段参与上下文，不全文注入；
9. 参考文件有索引时优先检索，无索引时显示 AI 自主查询的来源记录；
