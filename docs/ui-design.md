...
> 本文定义目标界面与交互，不代表界面已经接通真实后端。静态原型、模拟历史数据和可点击演示必须与真实功能分开标注，当前实现状态以 [设计与实施状态](implementation-status.md) 为准。
...
项目参数写入应用内部的项目 SQLite，不在用户工作目录创建 JSON、数据库或任务文件。默认单轮预算为 `400` 次模型调用、`7200000` 毫秒和 `10` 次检索轮次；单次模型请求默认允许 `3600000` 毫秒。模型上下文窗口默认为 `1000000` Token，系统在本次完整请求输入达到 `97%` 即 `970000` Token 时停止继续扩张并执行两阶段机械压缩，不能把超限后的截断交给模型供应商。应用不设置累计输入、输出 Token 截止线，只持续计量实际消耗；默认不向模型请求传入 `max_tokens`，正文长度仅通过提示词约束，具体输出上限由模型服务决定。这些提示词约束不是用户可见正文的硬截断。上述默认值只扩大可恢复执行空间，不扩大证据候选、图展开或上下文证据体积；保存后的设置从下一轮推演开始生效，已经运行的任务继续使用启动时冻结的配置快照。模型服务入口在同一设置导航中可达，但模型 Profile 属于应用级配置，API Key 继续使用系统安全存储，不写入项目数据库。
...
│   └── 用户创建的文件夹和.md
├── 章节正文/                     [固定，不可删除、重命名、移动]
│   ├── 第一章 xxxx.md            [由独立章节标题字段派生]
│   └── 第二章 xxxx.md
└── 表现输出/                     [固定，不可删除、重命名、移动]
    ├── 描写规则/                 [应用级共享目录，用户可编辑；AI 不写入]
    │   ├── 自动.md
    │   └── 默认描写规则.md
    ├── 笔风规则/                 [应用级共享目录，用户可编辑；AI 不写入]
    │   └── 默认笔风规则.md
    └── 本作品描写/               [固定目录，无子文件夹；用户与 AI 可写 .md]
```

平台提供的默认 Markdown 文件可以被用户查看，但属于基础规则时只读。用户规则、设定集、参考文件、章节正文、描写规则、笔风规则和本作品描写中的 Markdown 可编辑；章节正文的编辑会触发正文修订和重新结算，不能静默覆盖旧版本。用户可以选择直接提交，或先让 AI 审核后再提交；审核只提供建议，用户可以忽略建议并强制提交。提交后的正文立即成为新的正式版本，图同步状态单独展示并可恢复。
...
| `本作品描写` | 是 | 是 | 否 | 是 | 用户内容可软删除；最多 8 个文件、各约 4000 字 |
...
`描写规则/自动.md` 是创作台描写下拉「自动」的调度文件。选「自动」时注入该目录**全部** Markdown：以 `自动.md` 决定同一章何时切换场面，其余文件当场面卡。`描写规则/感官描写.md` 与 `描写规则/去AI味.md` 是**始终注入**的基线（分别管感官落点时机、压机器腔套话）；用户下拉锁定某一场面卡时，仍会一并注入这两份基线。用户可继续在该目录创建规则；**AI 不得写入此目录**。具体感官细节或专属禁用词偏好可写在 `本作品描写/`。

`笔风规则/默认笔风规则.md` 默认要求保持作品当前已形成的语言连续性。用户可以新增 Markdown 规定节奏、句式、词汇倾向、叙述密度、语言习惯和禁用表达；没有适用用户规则或无法判断时使用默认笔风规则。创作台每轮只注入用户下拉选中的一份笔风规则；**AI 不得写入此目录**。

`描写规则` 和 `笔风规则` 是应用级共享目录：所有作品中的同名入口映射到应用数据目录 `shared-workspace/表现输出/`，编辑、导入和删除即时作用于同一规则库，不是每个作品各复制一份。目录树标记“共享”，状态栏展示实际共享文件路径。只有 `本作品描写` 属于单个作品。

内置预设位于代码仓库 `packages/prompt-contracts/resources/presentation/表现输出/`，包含 41 份笔风和 11 份描写文件，随开发代码和安装包分发。初始化只安装未安装过的预设，不覆盖共享库中的修改，也不会在重启后重新创建已删除的预设。共享不等于跨设备实时云同步；设备间分发内置预设靠代码/安装包更新，自定义修改保存在各设备应用数据目录。

升级时扫描已注册作品的旧目录并执行一次迁移；打开其他旧作品时同样迁移。同名同内容去重，同名不同内容保留为 `原名-迁移-内容摘要.md`，原项目文件原样保留作备份，之后不再从备份反复导入。共享规则不进入作品历史快照，恢复旧快照时也忽略其中的两类规则，防止回滚某个作品时修改全局规则。

`本作品描写/` 是本作品持续生效的呈现附加层。用户与 AI 可在讨论后写入直接子级 `.md`（最多 8 个、每个不超过约 4000 字）；每轮全部注入。与选中描写/笔风冲突时以下拉文件为准。单章一次性氛围写在梗概/细纲，不写在本目录。溢出必须可见失败，不能静默跳过。

表现规则只控制正式正文如何表达，不自动改变世界事实、人物认知、事件结果或图结构。若用户在表现规则中同时写入世界变化要求，AI必须在 `interpret` 阶段把它拆分到世界推演约束，不能因文件位置而跳过出现规划和图治理。

### 5.6 章节正文

每次正式正文推演成功后，AI必须生成一个章节 Markdown 文档并写入 `章节正文` 根目录。章节标题是独立的纯文本字段；文件名和 Markdown 第一行都由代码使用同一个 `heading` 派生或装配：
...
- 文件名格式为 `第一章 xxxx.md`，其中章节序号和 `xxxx` 由 `chapter_naming` 返回的独立 `heading` 提供，文件名由代码合法化派生；
- `第一章 xxxx` 由代码在正式 Markdown 文档第一行装配，也是第一个 `NarrativeSourceUnit`；用户正文编辑器只编辑标题字段和标题以下的正文，不编辑 Markdown 标记；
...
- 章节编辑器只提供“保存修订”“查看差异”“直接提交”“审核后提交”“放弃修订”相关动作；普通保存只更新 `pending` 修订，不改变 `committed` 正文；
- 审核结果出现后，用户可以继续修改并重新审核，也可以直接提交或按当前审核结果提交；审核建议不能阻止提交，忽略建议提交时显示并记录“用户强制修改”；
- 正文提交与世界图同步状态分开显示。正文已提交而图同步失败时，允许重试或暂停，但默认不允许依赖未完成图同步继续新一轮推演；
- 用户修改独立章节标题字段时，提交边界使用新的 `heading` 同步更新数据库、章节索引、文件名和 Markdown 第一行；系统不从正文解析标题，也不接受正文首行作为标题输入；
...
> **与章节修订的边界：** 「本轮推演输入」仅在创作台（未打开章节 Tab）显示。打开已提交章节时，底部改为章节对话或修订面板，见 [章节修改入口协调设计](chapter-modification-coordination.md) §7。

正文阅读 Tab 下方固定保留本轮输入区（**仅创作台**）：
...
流程面板顶部的运行摘要固定保留四项核心指标：执行时间、活动上下文长度、KV 缓存平均命中率和上下文压缩总次数。四项指标统一使用圆环并排显示；前两者的一圈分别代表本轮允许的墙钟时间和模型上下文窗口，命中率的一圈代表 100%，压缩总次数只显示活动模型链已经执行的累计批次数，不伪造固定上限。指标名称、当前值和上限说明仅在悬浮或键盘聚焦时显示，避免占用流程列表空间。活动上下文长度在模型请求完成后使用提供方返回的最近一次真实输入 Token 更新；尚无已完成请求时显示“不可用”，不将本地字符估算展示为真实 Token。本地估算仅保留给请求前的上下文压缩保护。命中率只统计本轮模型调用返回的缓存输入 token：`cacheHitInputTokens / totalInputTokens`；供应商未返回缓存明细时显示“不可用”，不能显示为 `0%`。后端仍持续采集完整指标，包括阶段检索轮次；前端只将这四项置于流程列表上方，其余指标通过检查点和日志保留。

每个阶段行显示左侧状态图标和五个紧凑圆环符号，阶段行不再显示重复的文字状态列；悬浮或键盘聚焦圆环时显示该阶段的运行指标：平均上下文请求 Token 数、平均 AI 请求数、平均 KV 缓存命中率、平均请求时间和当前阶段检索轮次。同一阶段存在多次尝试或格式修复请求时，上下文 Token 和请求时间按提供方返回的真实用量除以真实模型调用次数加权计算，AI 请求数按阶段运行次数平均；KV 命中率按命中输入 Token / 总输入 Token 加权计算；检索轮次只统计该阶段实际返回 `request_read` 并进入资料读取的运行次数，不把格式修复或普通重试算作检索。阶段尚未产生模型运行数据时显示灰色 `--`，机械步骤不伪造 AI 指标。阶段结果同时保存思考文本和来源类型：供应商暴露推理文本时显示“AI 思考”，OpenAI Responses 只返回 reasoning summary 时显示“AI 思考摘要”；两者均按 Markdown 安全渲染，不执行原始 HTML。`reason`、`selfReview`、`requestedReads` 和 `unresolvedDependencies` 属于模型最终返回的结构化业务输出，随“AI 输出”展示，不因思考面板拆分而丢弃。

#### 7.1.1 运行监控

“本轮消耗”下方固定显示可折叠运行监控。后端动态返回完整运行指标，前端核心摘要只显示上述四项，保证流程列表有足够空间；模型调用、输入/输出 Token、网络重试和格式修复仍由后端收集并保留在检查点与日志中。
...
模型请求期间手动保存时，界面明确显示“已保存最近稳定检查点，当前请求仍在继续”。恢复任何包含检查点的保存点后，后端从该检查点克隆一个新的暂停任务，原任务和原世界线未来保持不变；流程 Tab 显示“已从历史恢复 · 已暂停”，不会自动请求模型，也不会自动重置额度。
...
5. `描写规则` 和 `笔风规则` 默认存在并可编辑；`本作品描写` 默认存在，仅允许直接子级 `.md`；
...
17. 章节编辑器提供独立的“章节标题”输入框；提交时数据库、文件名和 Markdown 第一行都由同一个 `heading` 字段生成，正文编辑器不包含标题行；
18. 编辑已提交章节会进入正文修订流程，不能静默覆盖旧版本；用户可以直接提交或审核后提交，AI建议不能阻止用户提交，用户强制修改的变化原因必须可查看；
