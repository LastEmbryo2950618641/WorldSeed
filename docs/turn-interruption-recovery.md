...
真实 `deepseek-v4-pro` 推演已经验证当前实现存在直接丢弃执行成果的问题：
...
检索轮次达到上限已经使用统一检索缺口继续执行，不属于任务中断。

## 3. 核心原则

### 3.0 单次模型请求也必须受控

整轮墙钟上限不能替代单次模型请求上限。DeepSeek 请求使用项目参数 `execution.maxModelRequestTimeMs` 生成独立的 AbortSignal，并与整轮截止时间取较短值。请求超时属于可恢复的外部执行中断：已完成阶段、上下文读取账本、原生思考、用量和 pending 作用域继续保留，任务进入 `awaiting_user_decision`，由用户选择继续、重试当前阶段或暂停本轮。
...
- `compact_context`：由用户明确启动上下文压缩和压缩复核，成功后建立新上下文窗口；
...
3. 写入 `ready_to_commit` 检查点；
4. 在 SQLite 事务中提交作用域，并把 finalization 标记为 `world_committed`；
5. 通过临时文件和原子替换发布章节 Markdown；
...
项目打开时扫描 `running`、`waiting_*` 和 `committing` 任务：
...
项目重新打开时，前端通过 `turn.recoverable.list` 获取等待决定或暂停任务，而不是依赖 React 内存中的任务对象。当前 V1 只恢复已经持久化为可恢复状态的任务；运行中的孤立任务扫描与租约恢复仍属于后续增强，不得在 UI 中伪装成自动继续。
...
运行监控固定在“流程”Tab 顶部，可折叠但不能被阶段列表滚动隐藏。每个可重置指标使用稳定单行布局：

```text
运行监控

执行时间       12:48 / 13:00        98%   [重置]
模型调用          10 / 63            16%   [重置]
输入 Token     97.5k / 不限制              [不可重置]
输出 Token     43.7k / 模型限制             [不可重置]
动态上下文       6.3k / 950k          1%   [压缩重置]
检索轮次            3 / 4             75%   [重置]
网络重试            0 / 2              0%   [重置]
格式修复            0 / 2              0%   [重置]

累计：97.5k 输入 / 43.7k 输出 / 12:48 / KV 命中 8%
                                      [全部重置]
```
