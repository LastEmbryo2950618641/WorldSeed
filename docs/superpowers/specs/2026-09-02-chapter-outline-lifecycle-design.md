...
2. **梗概确认（细纲前置门禁）**：戏核收窄并写入梗概后，须用户点选 `confirm_synopsis`（结果向文案如「用这份梗概写细纲」）或等价确认语；**未确认前不得写入 `[剧情细纲]`**。同屏可提供「跳过细纲，按梗概开推」→ `start_turn`（仅梗概兼容）。此确认 ≠ 设定集 `promote_staging`。
3. **细纲就绪**：确认后 Agent（主路径）或用户写出 `[剧情细纲]`，`stage=outline`；梗概**不删**，树折叠到细纲下。梗概若被实质性改写，确认位清零，须重新确认。
4. **开始推演**：须用户确认；注入见 §7。无细纲时允许仅用梗概（兼容），对话选项与高级菜单均可走「跳过细纲」。
5. **发布成功**：写入正式正文；`stage=body`；**不再删除**梗概/细纲文件；DB 仍可冻结快照（`ChapterSynopsis` + 新增 outline 快照可选）。
6. **正文回退**：撤销「正文为表面」——`stage=outline`；树主入口回到细纲；正文文件处理见 §8（不回到仅梗概）。
...
- 梗概戏核收窄后，Agent **先**写梗概并给出同屏三岔路：`confirm_synopsis`（写细纲）/ 跳过细纲开推 / 再改梗概；**用户确认写细纲后**再产出/更新 `[剧情细纲]`（`outlineBody`）；
- 细纲不是手填主路径；与推演目标一样：**Agent 写、用户审**；
- 「用这份梗概写细纲」与「把草案写入设定集」是两条独立确认链。

### 9.2 细纲推荐结构（提示词模板，非硬 schema）

必填：**章定位、分场节拍、信息边界、风险与待决**。  
按需（不适用写「本节：无」）：人物与关系、格局、与推演目标。  
细纲须相对梗概有场级增量；禁止同构扩写。完整示例与反例见 `plot-synopsis-guide.md` §4.1。
...
## 人物与关系
（有戏份人物；性格如何限制当场选择；无则「本节：无」）

## 格局
（当场力量关系；无外部势力时写明；禁止为填标题虚构）
...
- `synopsis-discuss.md` / `plot-synopsis-guide.md`：梗概后进入细纲（或同屏跳过）；有细纲后讨论默认改细纲；开推前提示以细纲为准；
