...
探针执行超过 Token、时间或模型调用预算时，按任务检查点协议暂停，不丢弃治理结果、探针计划或已完成探针。仅当选择性读取轮次达到 `maxRetrievalRounds` 时，应用将尚未执行的探针登记为统一 `system:retrieval-gap`，继续后续审核和提交；这类探针没有执行记录，也不能生成 `pass` 或 `uncertain` 评估。继续执行只运行尚未完成且仍未被检索轮次限制耗尽的探针；重试当前阶段从计划子步骤重新开始。后续轮次或用户修改触发新治理时，旧探针记录继续作为审计历史保存，但不能冒充新治理结果的验证证据。
...
`semantic_review.approvedAffectedFrontierRefs` 中每个引用必须恰好出现一次，不能增加、遗漏或重复；存在任何 rejected 前沿时不能进入本阶段。活跃或推迟前沿必须具有自己的最后场景、时间与地点锚点以及非空 `revisitCondition`；归档前沿继续保留返回路径，但可以省略重访条件。`disposition` 只用于调度，所有世界时空含义仍存在于普通图中。系统处理时间只能用于调度字段，不能写入 `lastTimeAnchorRefs`。正文场景存在时，锚点只能来自本轮获批的场景绑定；无正文后台演化时，阶段投影会携带本轮实际读取的同一前沿旧锚点，AI 可以继承这些锚点，但不能借用其他可读节点，也不能临时改变节点的时空用途。
