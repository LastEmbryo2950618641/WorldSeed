...
请求前没有统一精确 Token 接口时，机械压缩继续使用偏保守的本地估算，并且该估算不得作为真实用量展示。模型请求完成后，监控、阶段统计和额度累计统一使用提供方 `usage` 返回的真实输入、输出与缓存 Token；尚未返回 `usage` 时显示不可用。机械压缩会移出所有允许移出的旧轮次内容；如果旧模型的实际上下文窗口过低，导致系统规则、当前有效规则和当前轮保护内容本身超过该模型上限，则该模型不满足项目的上下文能力要求，任务暂停并提示用户切换支持更大上下文的模型。正常支持项目配置窗口的模型不会因为压缩目标本身而无限循环。
...
- 接收已经完成的可见消息序列；
- 原样发送给供应商；
- 返回正式内容、供应商可展示的思考文本或思考摘要、Token 与缓存统计；
