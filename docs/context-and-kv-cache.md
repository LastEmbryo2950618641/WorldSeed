...
`TurnContext` 是本轮上下文的持久化账本，不把模型 reasoningContent 当作世界事实或语义 artifact。供应商若单独返回 reasoningContent，应用会在阶段运行记录中独立保存，界面默认折叠展示；上下文账本只保存阶段输入、阶段输出摘要、实际读取 ID、引用关系、摘要、token 估算和缓存统计。
...
每次模型调用明确分为三类。三类是装配边界，不代表需要创建三套上下文：

1. **常驻系统提示词**：平台锁定的基础规则 Markdown（完整内容或其固定规范化版本）、通用 Prompt Contract、底层一致性规则和协议纪律。用户可见但不可修改。
2. **动态提示词**：当前项目目录快照、动态规则和资料读取结果，包含目录驱动的文件读取和 AI 选择性读取的世界图局部。
3. **用户回合提示词**：用户前置提示、用户本轮输入、描写规则、笔风规则、字数上下限和固定后置强调，每轮重新追加。

### 3.2 动态目录和资料入口

每轮先生成目录快照。目录快照只包含路径、类型、版本或摘要和入口，不包含章节正文全文，也不自动把所有设定和参考文件读入模型。

- `世界推演规则/用户规则` 下的 Markdown 每轮全部读取；
- `设定集/readme.md` 和 `参考文件/readme.md` 是不可删除但可编辑的强制索引入口；
- 设定集和参考文件先读取对应 readme，再按当前任务选择性读取；
- 缺少任一强制 readme 时，项目校验失败，不开始正式推演；
- `表现输出` 不进入默认资料检索；用户选择的描写规则和笔风规则在首轮被实际读取，作为 `presentation_rules` 片段和本轮结构化表现参数注入；
- `章节正文` 不进入默认上下文，只在明确历史查询或检索需要时读取。

每轮上下文按以下顺序构造，并尽量保持前缀稳定：

```text
锁定的基础规则 Markdown
→ Prompt Contract 公共部分
→ 项目目录快照
→ 用户规则全部 Markdown
→ 设定集/readme.md
→ 按 readme 选择的设定片段
→ 参考文件/readme.md
→ 按 readme 选择的参考片段
→ AI 选择性读取的 committed/pending 局部图
→ 用户前置提示
→ 用户本轮输入
→ 已选择的描写规则、笔风规则正文（`presentation_rules`）
→ 字数上下限等结构化表现参数
→ 用户后置强调
→ 本轮新产生的阶段结果
```

其中“表现输出”规则文件只有用户本轮明确选择的文件会被读取，不作为世界事实或默认资料集合；“章节正文”始终不在默认装配中，只有历史查询或 AI 明确提出并完成检索时才进入实际读取集合。
...
所有阶段必须通过同一个 `ContextAssembler` 按上述顺序构造模型请求；编排器、检索器和模型适配器不能分别维护自己的 Prompt 拼接规则。动态读取轮次、工作区读取轮次、图读取轮次、请求数、证据 token 和耗时由同一个 `turnId` 级预算账本累计，不能在阶段重试时重新计数。
...
1. 当前场景的 `SceneSpacetimeBinding`、实际前置场景和必要过渡路径；
2. 本轮涉及当前状态的 `MutationSpacetimeSettlement`、前置修订和历史返回入口；
3. 当前人物或行动主体实际可用的信息；
4. 形成当前行动所必需的已读取事实；
5. 用户本轮输入和表现规则；
6. 尚未完成的依赖闭包；
...
多时间流和动态空间不进入常驻上下文。普通场景只保留当前局部时间、空间和前置场景；只有任务发生跨参照移动、比较或追溯时，才按 [通用时空锚点设计](spacetime-anchor-design.md) 追加相关同步点、对应结构或不确定性结构。其他世界、其他时间线和无关地图继续留在持久化图中。
...
历史动态区的语义摘要必须由 AI 生成压缩提案，并由独立 AI 复核其时间、空间、状态、否定条件、认知边界、未解决依赖和返回路径；代码只检测预算、校验引用并持久化检查点。平台基础规则和当前回合保护区不参与压缩。用户规则只允许无损规范化，不能用有损摘要替代；无损规范化后仍超出模型窗口时阻止正式推演。
...
- 压缩前后当前状态、场景时空绑定、前置场景和过渡路径不发生无依据回退；
