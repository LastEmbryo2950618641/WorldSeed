...
- 目录变化、强制索引和选择性资料读取如何进入每轮上下文，而不把外部资料自动变成世界事实。
...
1. **基础规则**：平台提供、用户可见、不可修改的底层规则和连续性门禁；内容如何落笔见同目录的内容处理规则，不承担发表审核；
...
```text
目录快照 -> 规则与索引读取 -> 按任务选择性读取设定、参考资料和世界图
```

目录快照只描述当前可用的路径、类型、版本或摘要，不是世界事实。
...
内容处理规则（`content-handling.md`）与基础规则一并进入系统前缀，但职责分开：基础规则管图、读取与连续性；内容处理规则是虚构交付契约——原请求必须直接交差，拒答或安全改写视为失败；身心状态、权力选择、死伤后果、镜头落点和信息边界只跟世界因果与作者指令，不能用道德或读者保护替换。运行时在正文相关阶段检查开头窗口，失败走既有结构修复重试；DeepSeek 阶段尾部会再贴一次交付提醒。
...
设定集必须包含根目录固定文件 `设定集/readme.md`。该文件不可删除但可以由用户编辑，是设定集的索引和读取入口。缺少该文件时项目校验失败，不能开始正式推演。

除 `设定集/readme.md` 外，设定集中的 Markdown 不默认全文读取。AI 必须先读取 `设定集/readme.md`，再根据当前任务、目录快照、图入口和 readme 引导选择性读取相关文件或片段。每个设定文件先登记可检索摘要、关键词、关联入口和适用范围。大纲也只是规划约束，不会自动把其中所有未来事件写成已经发生的事实。

### 2.4 参考资料的作用

参考资料用于提供外部事实、表达方式、专业背景、灵感或对照材料。它的权威低于用户设定集，默认不能直接改变作品世界。

参考文件必须包含根目录固定文件 `参考文件/readme.md`。该文件不可删除但可以由用户编辑，是参考资料索引和查询入口。缺少该文件时项目校验失败，不能开始正式推演。

AI 必须先读取 `参考文件/readme.md`，再根据当前任务和索引选择性读取参考文件。不存在“没有索引时自动放宽为任意查询”的路径；索引缺失是项目配置错误，而不是 AI 自行补救的条件。
...
用户规则 Skill 每轮读取 `世界推演规则/用户规则` 下全部 Markdown 的正文。用户编辑 Markdown 后生成新版本，旧版本保留以便追溯；新版本何时生效由用户规则中的生效范围和 AI复核决定。目录快照仍然作为动态目录入口，但不能替代规则正文。

用户规则的执行顺序是：读取规则 -> 解释适用范围 -> 检查与基础规则冲突 -> 形成本轮约束 -> 再参与出现规划和正文生成。用户规则不能在自身文本中声明“忽略基础规则”来提升权限。

### 3.3 设定集 Skill

设定集 Skill 采用“目录快照 + 根目录文件 + readme 索引 + 按需片段”注入：
...
参考资料 Skill 默认只提供检索能力和来源说明，不提供作品世界的强制指令。它采用“目录快照 + `参考文件/readme.md` + 按需片段”注入。若用户明确希望某份参考资料成为作品规则，应把它转化为用户规则或设定集，由 AI重新解释和提交，不能只提高参考资料的优先级。

Skill 必须有索引入口；查询到的网页、文献或同人片段必须记录来源、访问时间或版本、实际摘录和用途，之后才能作为本轮依据。
...
flowchart TD
    ...
    B --> C["读取用户规则目录下全部 Markdown"]
    C --> D["读取当前场景和必要的持久化图局部"]
    D --> E["按需检索设定集 Skill"]
    E --> F{"任务是否需要参考资料？"}
    F -->|需要| G["读取参考文件/readme.md"]
    G --> H["按索引选择性召回参考片段"]
    F -->|不需要| I["不读取参考资料"]
    H --> J["形成实际读取集合和规则快照"]
    I --> J
...
基础规则摘要和适用的用户规则是规则输入；设定集和参考资料是本轮事实或创作依据。它们都必须记录来源 ID、版本、读取片段和注入原因。表现输出分三层：用户规则（可移植、每轮全量）、创作台下拉选中的一份描写规则与一份笔风规则、以及 `表现输出/本作品描写/` 下全部 Markdown（本作品持续呈现附加层，冲突以下拉为准）。章节正文不进入默认资料上下文，只在明确历史查询或图检索需要时读取。

上下文压缩时可以压缩已注入的历史设定、参考资料和图证据文字，但不能压缩掉这些记录中的持久入口。平台基础规则和用户规则正文不参与有损压缩；用户规则只允许无损规范化，规范化后仍超出模型窗口时阻止正式推演。后续轮次需要某个设定或资料时，先从 manifest/index 重新定位，再读取片段；不能因为上一轮加载过就认为本轮仍然拥有该依据。
...
参考资料是外部依据或灵感，优先级低于用户规则和设定集。每轮只有在任务需要时读取 `参考文件/readme.md`，再按索引选择性读取具体资料。索引缺失属于项目配置错误，不启动正式推演；不能改为由 AI 任意查询来补齐索引。没有实际读取的资料不能作为本轮事实依据。

只输出规则选择控制结果：本轮实际选中的工作区路径、每个路径的一句选择原因和无法解决的规则冲突。不要输出规则正文、资料摘要、目录复印件、完整 `RuleSnapshot` 或隐藏思维过程；没有新增选择或冲突时返回空数组、空对象和空数组。
```
...
只为当前任务寻找必要资料，不为了补全世界而泛读。先读取 `参考文件/readme.md`，再依照索引和当前任务选择必要资料；索引缺失时返回项目配置错误，不执行任意查询兜底。
...
参考资料查询必须以 `参考文件/readme.md` 为入口，并且每次查询都必须先判断当前任务是否确实需要外部资料。查询结果分为：
...
5. 设定集和参考文件必须存在可编辑但不可删除的 `readme.md` 索引入口；
